...
public class TwoYellowToteAuto implements AutonomousProcedure
{
	...
	public void doAuto() 
	{
		...
		iDriverAssist.toteSweetSpot(); // Moves toward tote that is placed in front of the robot at the beginning of a match
		iDriverAssist.correctRotation(); // Correct rotation in front of the tote
		
		forkLift.nextToteLength(); // Pick up tote
		forkLift.nextToteLength(); // Go up one more so we can get the second tote easily
		
		// Move backward slightly and then move in the X direction until we reach the second yellow tote
		...
		mecanumDrive.stop(); 
		mecanumDrive.move(1.0, 90, 0); // Move at a 90 degree angle to the other tote
		
		List<Obstacle> obstacleList = cameraAssist.getObjects();
		
		// Wait until the previous tote gets out of vision
		// TODO: replace with a closed loop system (detect when one tote leaves and when the other enters)
		try {
		...
		iDriverAssist.toteSweetSpot(); // get square with the tote and drive up to it correctly
		
		forkLift.goToBottom(); // Put tote back on the other tote and go back down to the bottom position so we can lift the whole stack up
